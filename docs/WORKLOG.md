...
- 开始页主标题改为「萌萌拼图」（系统粗体+粉色，Great Vibes 无中文字形）；"MM Puzzle" 花体缩成 64px 装饰副标题保留在标题上方
- 备案名要和游戏内显示一致，提审截图注意用新标题
- wechatgame 已重新构建 + 分包后处理
...
- 症状（仅线上，编辑器预览没感觉）：开始页没背景图、点 Start 无反应、点 Random 能玩
- 根因：`detectLevels` 原来是**串行**探测（source1 下载完才试 source2……直到第一个不存在的），17 张 900×1200 整图逐张从 github.io 拉，国内访问要几十秒。期间开始页背景（等检测完才设置）和选关页（等检测完才建）都没就绪；Random 不依赖检测结果所以能玩
- 修复（`PuzzleGame.ts`）：
  - `detectLevels` 改为**并行**探测 source1~24，全部回调或 10 秒超时后按「连续存在」的关数结算
  - source1 整图一到就先铺开始页背景，不等其余图片
  - 开始页加 `Loading...` 提示（检测完隐藏），网络慢时点 Start 没反应至少有解释
- 教训：**编辑器预览全走本地磁盘，串行加载瞬间完成；任何「加载完才显示」的逻辑上线后都会被真实网络放大**，探测类加载要并行 + 超时兜底 + 中间态提示
...
- **⚠️ 构建后必做三处**（少一个线上就挂）：
  1. Pages 挂在子路径 `/mmpz/` 下，而 Cocos 构建默认 `"server":""`（资源从网站根路径加载 → 全 404，游戏有界面没图）。每次构建完要把 `build/web-mobile/src/settings.json` 里 `assets.server` 改成 `"/mmpz/"`
  2. **`touch .nojekyll`**！Pages 默认跑 Jekyll，会吞掉所有下划线开头的文件（`cocos-js/_virtual_cc-xxx.js` 404 → 引擎直接加载失败）。重建 git 仓库时最容易忘
  3. **重新构建会清空 `build/web-mobile`（连同里面嵌套的 .git）**，所以每次部署要重新 `git init -b gh-pages` + `git remote add origin` + commit + `push -f origin gh-pages`，不要以为嵌套仓库还在
